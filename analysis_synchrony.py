...
def main(model, condition):
    ...
    features = pd.read_csv("data/features_synchrony.csv")
    ...
    # Save results to CSV
    results.to_csv(f"synchrony/results_{model_name}{f"_{condition}" if condition != "single" else ""}.csv", index=False)
    print("Wrote synchrony/results.csv")
...
def mtp_main(model, condition):
    ...
    out_name = f"synchrony/results_{model_name}{f'_{condition}' if condition != 'single' else ''}_mtp.csv"
    results.to_csv(out_name, index=False)
    print(f"Wrote synchrony/{out_name}")
    return results

if __name__ == "__main__":
    for model in ["gpt-oss:20b", "gpt-oss:120b", "deepseek-v3.1:671b", "llama3:8b"]:
